...
{  
	...
	while (!exitapp) 
	{
		...
		while (SDL_PollEvent( &event )) 
		{
			...
			game->KeyA(keys[SDL_SCANCODE_A]);
			game->KeyD(keys[SDL_SCANCODE_D]);
			game->KeyW(keys[SDL_SCANCODE_W]);
			game->KeySpace(keys[SDL_SCANCODE_SPACE]);
			game->KeyEnter(keys[SDL_SCANCODE_RETURN]);
		}
	}
	game->Shutdown();
	SDL_Quit();
